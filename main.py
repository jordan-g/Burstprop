...
parser.add_argument("-num_hidden_layers", type=int, help="Number of hidden layers.", default=1)
...
parser.add_argument("-num_hidden_units", help="Number of units in each layer.", type=lambda s: [int(item) for item in s.split(',')], default=[500])
parser.add_argument("-Z_range", help="Range of uniform distribution used for initial recurrent weights.", type=lambda s: [float(item) for item in s.split(',')], default=[0.01])
parser.add_argument("-Y_range", help="Range of uniform distribution used for initial feedback weights.", type=lambda s: [float(item) for item in s.split(',')], default=[1.0])
parser.add_argument("-forward_learning_rates", help="Feedforward learning rates.", type=lambda s: [float(item) for item in s.split(',')], default=[0.05, 0.01])
parser.add_argument("-recurrent_learning_rates", help="Recurrent learning rates.", type=lambda s: [float(item) for item in s.split(',')], default=[0.0001])
parser.add_argument("-gamma", type=float, help="Output layer burst probability in the absence of a target.", default=1)
...
parser.add_argument("-same_sign_weights", type=bool, help="Whether to use feedback weights that are the same sign as feedforward weights.", default=True)
...
parser.add_argument("-use_validation", type=bool, help="Whether to use the validation set.", default=False)
parser.add_argument("-use_backprop", default=True, type=lambda x: (str(x).lower() == 'true'))
...
def train(x_train, d_train, x_test, d_test, parameters, state, gradients, hyperparameters):
	...
	for epoch_num in range(hyperparameters["num_epochs"]):
		for batch_num in range(num_batches):
			...
			x = x_train[:, batch_num*hyperparameters["batch_size"]:(batch_num+1)*hyperparameters["batch_size"]]
			d = d_train[:, batch_num*hyperparameters["batch_size"]:(batch_num+1)*hyperparameters["batch_size"]]
			...
			parameters = net.update_weights(parameters, gradients, hyperparameters)
			...
			if (batch_num+1) % hyperparameters["test_frequency"] == 0:
				...
				print("Epoch {}, batch {}/{}. {} error: {:.2f}%.".format(epoch_num+1, batch_num+1, num_batches, "Validation" if hyperparameters["use_validation"] else "Test", test_error[total_batch_num+1]))
